...
class AuthController extends Controller
{
    ...
    public function login(
        ...
        $credentials = $request->only('username', 'password') + ['role' => [User::ROLE_ADMIN, User::ROLE_SUB_ACCOUNT]];
        ...
        abort_if(
            !$whitelistedIpManager->isAllowedToLoginFromRequest($request),
            Response::HTTP_BAD_REQUEST,
            __('IP 未加入白名单 :ip', ['ip' => $whitelistedIpManager->extractIpFromRequest($request)])
        );
        ...
        $notificationUtil->notifyAdminLogin(auth()->user()->realUser(),
