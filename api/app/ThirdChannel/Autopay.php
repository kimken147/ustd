...
class Autopay extends ThirdChannel
{
    //Log名称
    public $log_name = 'Autopay';
    public $type = 1; //1:代收付 2:纯代收 3:纯代付

    //回调地址
    public $notify = '';
    public $depositUrl = 'http://pay-gateway.988autopay.com/api/pay/create_order';
    public $xiafaUrl = '';
    public $daifuUrl = 'http://pay-gateway.988autopay.com/api/trans/create_order';
    ...
    public $queryBalanceUrl = 'http://pay-gateway.988autopay.com/api/query/query_balance';
    ...
    public function sendDeposit($data)
    {
        ...
        $postBody = [
            "mchId" => $data["merchant"],
            'appId' => $this->key,
            'productId' => $data['key4'] ?? $this->channelCodeMap[$this->channelCode],
        ...
        if (isset($row['retCode']) && in_array($row['retCode'], ['SUCCESS'])) {
            $ret = [
                'pay_url' => $row["payUrl"] ?? '',
                'receiver_name' => $payParams["bankUsername"] ?? "",
                'receiver_bank_name' => $payParams["bankName"] ?? "",
                'receiver_account' => $payParams["bankNo"] ?? "",
                'receiver_bank_branch' => $payParams["bankNumber"] ?? "",
            ];
