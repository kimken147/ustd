<?php

namespace App\ThirdChannel;

use App\Model\Channel;
use App\Model\Transaction;
use App\Model\ThirdChannel as ThirdChannelModel;
use Exception;
use Illuminate\Support\Facades\Log;
use GuzzleHttp\Exception\RequestException;
use GuzzleHttp\Client;

class Jihefu extends ThirdChannel
{
    //Log名称
    public $log_name   = 'Jihefu';
    public $type    = 1; //1:代收付 2:纯代收 3:纯代付－

    //回调地址
    public $notify    = '';
    public $depositUrl   = 'http://zhifu.meiannaisi.com/mapi.php';
    public $xiafaUrl   = '';
    public $daifuUrl   = '';
    public $queryDepositUrl    = '';
    public $queryDaifuUrl  = '';
    public $queryBalanceUrl = 'http://zhifu.meiannaisi.com/api.php';

    //默认商户号
    public $merchant    = '';

    //默认密钥
    public $key          = '';
    public $key2         = '';

    //回传字串
    public $success = 'success';

    //白名单
    public $whiteIP = ['34.92.161.2'];

    public $channelCodeMap = [
        Channel::CODE_BANK_CARD => "BankToBank",
    ];

    public $bankMap = [];

    /*   代收   */
    public function sendDeposit($data)
    {
        $this->key = $data['key'];
        $postBody = [
            "pid" => intval($data["merchant"]),
            "type" => 'alipay',
            'out_trade_no' => $data['request']->order_number,
            'notify_url' => $data['callback_url'],
            'name' => 'VIP会员',
            'money' => sprintf("%.2f", (float)$data['request']->amount),
            'clientip' => $data['request']->client_ip ?? $data['client_ip'] ?? $this->generateRandomIp(),
            'sign_type' => 'MD5'
        ];

        try {
            $row = $this->sendRequest($data["url"], $postBody);
            $ret = [
                'pay_url'   => $row["payurl"] ?? $row['qrcode'] ?? '',
            ];
            return ['success' => true, 'data' => $ret];
        } catch (\Throwable $th) {
            return ["success" => false, "msg" => $th->getMessage()];
        }
    }

    public function queryDeposit($data)
    {
        return ['success' => true, 'msg' => '原因'];
    }

    /*   代付   */
    public function sendDaifu($data)
    {
        return ["success" => false, 'msg' => '通知不支援代付'];
    }

    public function queryDaifu($data)
    {
        return ['success' => false];
    }

    /*   回调 => callback($request,订单资料)   */
    public function callback($request, Transaction $transaction, ThirdChannelModel $thirdChannel)
    {
        $data = $request->all();

        if ($data["out_trade_no"] != $transaction->order_number) {
            return ['error' => '订单编号不正确'];
        }

        if ($data["money"] != $transaction->amount) {
            return ['error' => '代收金额不正确'];
        }

        //代收检查状态
        if (in_array($data["trade_status"], ['TRADE_SUCCESS']) && in_array($transaction->type, [1])) {
            return ['success' => true];
        }

        // //代付检查状态，失败
        // if (in_array($data["status"], [4])) {
        //     return ['fail' => '逾时'];
        // }

        return ['error' => '未知错误'];
    }

    public function queryBalance($data)
    {
        $this->key = $data["key"];

        $postBody = [
            "pid" => $data["merchant"],
            'act' => 'query',
            'key' => $this->key
        ];

        try {
            $row = $this->sendRequest($data["queryBalanceUrl"], $postBody, 'GET', false);
            $balance = $row["money"];
            ThirdChannelModel::where("id", $data["thirdchannelId"])->update([
                "balance" => $balance,
            ]);
            return $balance;
        } catch (\Throwable $th) {
            $message = $th->getMessage();
            Log::error(self::class, compact('data', 'message'));
            return 0;
        }
    }

    private function sendRequest($url, $data, $method = "POST", $debug = true)
    {
        $data["sign"] = $this->makesign($data, $this->key);
        try {
            $client = new Client();
            $options = $method === "GET"
                ? ["query" => $data]  // GET 方法使用 query 參數
                : ["form_params" => $data];  // POST 方法使用 json 參數

            $response = $client->request($method, $url, $options);

            $row = json_decode($response->getBody(), true);
            if ($debug) {
                Log::debug(self::class, compact('data', 'row'));
            }
            if ($row['code'] != 1) {
                throw new Exception($row['msg']);
            }

            return $row;
        } catch (RequestException $e) {
            $response = $e->getResponse();
            $message = json_decode($response->getBody()->getContents());
            $message = $message->message ?? "";
            Log::error(self::class, compact('data', 'message'));
            throw new Exception($message);
        }
    }

    public function makesign($body, $key)
    {
        ksort($body);
        unset($body["sign"], $body['sign_type']);
        $signStr = urldecode(http_build_query($body) . "$key");
        return (md5($signStr));
    }

    private function generateRandomIp(): string
    {
        return mt_rand(1, 255) . '.' .
            mt_rand(0, 255) . '.' .
            mt_rand(0, 255) . '.' .
            mt_rand(0, 255);
    }
}
