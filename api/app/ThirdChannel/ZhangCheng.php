...
class ZhangCheng extends ThirdChannel
{
    //Log名称
    public $log_name = 'ZhangCheng';
    public $type = 1; //1:代收付 2:纯代收 3:纯代付

    //回调地址
    public $notify = '';
    public $depositUrl = 'https://pay.zhangcheng168.com/api/pay/create_order';
    public $xiafaUrl = '';
    public $daifuUrl = 'https://pay.zhangcheng168.com/api/trans/create_order';
    public $queryDepositUrl = 'https://inquiry.zhangcheng168.com/api/pay/query_order';
    public $queryDaifuUrl = 'https://inquiry.zhangcheng168.com/api/trans/query_order';
    public $queryBalanceUrl = 'https://inquiry.zhangcheng168.com/api/query/query_balance';
    ...
    public function sendDeposit($data)
    {
        ...
        $channelCode = $this->key4 ?? $this->channelCodeMap[$this->channelCode];
        $postBody = [
            "mchId" => $data["merchant"],
            'appId' => $this->key,
            ...
            "subject" => $channelCode,
            "body" => $channelCode,
            "param1" => !empty($data["request"]->real_name) ? $data["request"]->real_name : str_pad(rand(0, 999999), 6, '0', STR_PAD_LEFT),
        ];
        ...
        if (isset($row['retCode']) && in_array($row['retCode'], ['SUCCESS'])) {
            $payParams = $row["payParams"];
            $payUrl = $payParams["payUrl"];
            $ret = [
                'order_number' => $data['request']->order_number,
                'amount' => $data['request']->amount,
                'pay_url' => $payUrl ?? '',
                'receiver_name' => $payParams["bankOwner"] ?? "",
                'receiver_bank_name' => $payParams["bankName"] ?? "",
                'receiver_account' => $payParams["bankNo"] ?? "",
                'receiver_bank_branch' => $payParams["bankFrom"] ?? "",
            ];
