...
*/
(function(jQuery) {
	jQuery.jCryption = function(el, options) {
		var base = this;
		...
		base.init = function() {
			...
			base.options = jQuery.extend({},jQuery.jCryption.defaultOptions, options);

			$encryptedElement = jQuery("<input />",{
				type:'hidden',
			...
			$submitElement.bind(base.options.submitEvent,function() {
				...
				if(base.options.beforeEncryption()) {
					   jQuery.jCryption.getKeys(base.options.getKeysURL,function(keys) {
						   jQuery.jCryption.encrypt(base.$el.serialize(),keys,function(encrypted) {
						   	   $encryptedElement.val(encrypted);
						   	   jQuery(base.$el).find(":input").attr("disabled",true).end().append($encryptedElement).submit();
						   });
					   });
				}
				return false;
	...
	jQuery.jCryption.getKeys = function(url,callback) {
	...
	jQuery.jCryption.encrypt = function(string,keyPair,callback) {
		var charSum = 0;
	...
	jQuery.fn.jCryption = function(options) {
		return this.each(function(){
			(new jQuery.jCryption(this, options));
		});
	};
