...
const ProviderWhiteList: FC = () => {
  ...
  const {
    ...
    filters: {
      permanent: [
        ...
        { field: 'role', value: 2, operator: 'eq' },
  ...
  return (
    <>
      ...
      <List title={<ContentHeader title={t('titles.whiteList')} resource="providers" />}>
        <ListPageLayout>
          <ListPageLayout.Filter formProps={searchFormProps}>
            <Col xs={24} md={12}>
              <ListPageLayout.Filter.Item label={t('filters.nameOrAccount')} name="name_or_fuzzy_username">
                <Input allowClear />
