...
var serveClient = (app) => {
  ...
  app.use((req, res, next) => {
    ...
    res.status(200).sendFile(import_path.default.join(`${__dirname}/client/index.html`));
  });
};
...
var refineEnv = {};
import_dotenv.default.config({ processEnv: refineEnv });
...
var serveWs = (server2, onError) => {
  const ws = new import_ws.default.Server({ server: server2 }).on("error", (error) => {
    if ((error == null ? void 0 : error.code) === "EADDRINUSE") {
      console.error(
        `
${import_chalk.cyanBright.bold("\u2717 ")}${(0, import_chalk.bold)(
...
`
      ...
      console.error(
        `
${import_chalk.cyanBright.bold("\u2717 ")}${(0, import_chalk.bold)("error from refine devtools")}`,
        error
      );
    }
    ws.close(() => {
...
var import_debounce = require("lodash/debounce");
...
var setupServer = (app, onError) => {
  const server2 = import_http.default.createServer(app);
  server2.on("error", (error) => {
    if ((error == null ? void 0 : error.code) === "EADDRINUSE") {
      console.error(
        `
${import_chalk2.cyanBright.bold("\u2717 ")}${(0, import_chalk2.bold)(
          "Refine Devtools server"
        )} (http) failed to start. Port ${SERVER_PORT} is already in use.
`
      );
      console.info(
        `${import_chalk2.cyanBright.bold(
          "\u2139 "
        )}You can change the port by setting the ${(0, import_chalk2.bold)(
...
${import_chalk2.cyanBright.bold("\u2713 ")}${(0, import_chalk2.bold)(
        "Refine Devtools"
      )} is running at port ${import_chalk2.cyanBright.bold(SERVER_PORT)}
`
    );
  });
  process.on("SIGTERM", () => {
...
var contentToHtml = (content) => {
  ...
  renderer.link = function(...args) {
    const out = import_marked.marked.Renderer.prototype.link.apply(this, args);
    return out.replace(/^<a/, '<a target="_blank" rel="noopener noreferrer"');
  };
  const html = (0, import_marked.marked)(content, {
    renderer
  });
  return (0, import_sanitize_html.default)(html, {
    allowedTags: import_sanitize_html.default.defaults.allowedTags.concat(["img"]),
    allowedAttributes: {
...
var parseSection = (section) => {
  const parsed = (0, import_gray_matter.default)(section.replace("---section", "---"));
  return {
    ...parsed.data,
...
var fetchFeed = async () => {
  try {
    const response = await (0, import_node_fetch.default)(FEED_MD_URL);
...
var getInstalledPackageJSONPath = async (packageName) => {
  try {
    const filesFromGlobbySearch = await (0, import_globby.default)(
    ...
    try {
      const pkgJsonPath = require.resolve(import_path2.default.join(packageName, "package.json"));
      if (pkgJsonPath) {
...
var getInstalledPackageData = async (packageName) => {
  try {
    ...
    const parsed = JSON.parse(
      import_fs.default.readFileSync(packagePath, { encoding: "utf-8" })
    );
    return {
      name: parsed.name,
...
var getPackagesFromPackageJSON = async (projectPath = process.cwd()) => {
  const packageJson = await (0, import_fs_extra.readJSON)(import_path3.default.join(projectPath, "package.json"), {
    encoding: "utf-8"
  });
...
var updatePackage = async (packages, projectPath = process.cwd()) => {
  try {
    const detected = await (0, import_package_manager_detector.detect)({ cwd: projectPath });
    const [pm] = ((detected == null ? void 0 : detected.agent) || "npm").split("@");
    const { failed } = await (0, import_execa.default)(pm, [
      "install",
...
var getLatestPackageData = async (packageName) => {
  try {
    const { stdout } = await (0, import_execa2.default)("npm", [
      "view",
      packageName,
      "name",
      "version",
...
var getProjectIdFromPackageJson = async (projectPath = process.cwd()) => {
  var _a;
  try {
    const packageJson = await (0, import_fs_extra2.readJSON)(import_path4.default.join(projectPath, "package.json"), {
      encoding: "utf-8"
    });
...
var setProjectIdToPackageJson = async (projectId, projectPath = process.cwd()) => {
  try {
    import_execa3.default.sync("npm", ["pkg", "set", `refine.projectId=${projectId}`], {
      cwd: projectPath
    });
    return true;
  } catch (e) {
...
var setProjectIdToRefineComponent = async (projectId, projectPath = process.cwd()) => {
  try {
    ...
    const execution = import_execa4.default.sync(
      jscodeshiftExecutable,
      [
        "./",
        "--extensions=ts,tsx,js,jsx",
        "--parser=tsx",
        `--transform=${import_path5.default.resolve(
          import_path5.default.join(__dirname, "..", "src", "project-id", "transform.ts")
        )}`,
...
// src/serve-api.ts
var serveApi = (app, db) => {
  app.use("/api", (0, import_express2.json)());
  ...
  app.get("/api/unique-trace-items", (req, res) => {
    ...
    const uniqueTraceItems = (0, import_uniq.default)(traceItems);
...
var persistPath = import_path6.default.join(__dirname, "..", ".persist.json");
var saveAuth = async (token, jwt) => {
  try {
    await (0, import_fs_extra3.writeJSON)(persistPath, { token, jwt });
  } catch (error) {
  }
};
var loadAuth = async () => {
  try {
    return await (0, import_fs_extra3.readJSON)(persistPath);
  } catch (error) {
  }
  return {};
};
var serveProxy = async (app) => {
  ...
  const authProxy = (0, import_http_proxy_middleware.createProxyMiddleware)({
  ...
  const refineProxy = (0, import_http_proxy_middleware.createProxyMiddleware)({
...
var serveOpenInEditor = (app, basePath) => {
  app.get("/open-in-editor/*", (req, res) => {
    ...
    const vscodeUrl = `vscode://file/${import_path7.default.join(basePath, filePath)}?${line ? `line=${line}` : ""}${column ? `&column=${column}` : ""}`;
...
var server = async ({
  ...
  return new Promise((_, reject) => {
    ...
    const db = createDb();
    ws.on("connection", (client) => {
      (0, import_devtools_shared2.receive)(client, import_devtools_shared2.DevtoolsEvent.DEVTOOLS_INIT, (data) => {
        if (db.connectedApp) {
          (0, import_devtools_shared2.send)(client, import_devtools_shared2.DevtoolsEvent.DEVTOOLS_ALREADY_CONNECTED, {
            url: db.connectedApp
          });
        } else {
          ...
          ws.clients.forEach((c) => {
            (0, import_devtools_shared2.send)(c, import_devtools_shared2.DevtoolsEvent.DEVTOOLS_CONNECTED_APP, {
              url: db.connectedApp
            });
          });
        }
      });
      (0, import_devtools_shared2.receive)(client, import_devtools_shared2.DevtoolsEvent.ACTIVITY, (data) => {
        ...
        ws.clients.forEach((c) => {
          (0, import_devtools_shared2.send)(c, import_devtools_shared2.DevtoolsEvent.DEVTOOLS_ACTIVITY_UPDATE, {
            updatedActivities: [record]
          });
        });
      });
      (0, import_devtools_shared2.receive)(
        client,
        import_devtools_shared2.DevtoolsEvent.DEVTOOLS_HIGHLIGHT_IN_MONITOR,
        ({ name }) => {
          ws.clients.forEach((c) => {
            (0, import_devtools_shared2.send)(c, import_devtools_shared2.DevtoolsEvent.DEVTOOLS_HIGHLIGHT_IN_MONITOR_ACTION, {
              name
            });
          });
        }
      ...
      (0, import_devtools_shared2.receive)(
        client,
        import_devtools_shared2.DevtoolsEvent.DEVTOOLS_INVALIDATE_QUERY,
        ({ queryKey }) => {
          ws.clients.forEach((c) => {
            (0, import_devtools_shared2.send)(c, import_devtools_shared2.DevtoolsEvent.DEVTOOLS_INVALIDATE_QUERY_ACTION, {
              queryKey
            });
          });
        }
      );
      (0, import_devtools_shared2.receive)(client, import_devtools_shared2.DevtoolsEvent.DEVTOOLS_LOGIN_SUCCESS, () => {
        ws.clients.forEach((c) => {
          (0, import_devtools_shared2.send)(c, import_devtools_shared2.DevtoolsEvent.DEVTOOLS_RELOAD_AFTER_LOGIN, {});
        });
      });
      (0, import_devtools_shared2.receive)(
        client,
        import_devtools_shared2.DevtoolsEvent.DEVTOOLS_LOGIN_FAILURE,
        ({ error, code }) => {
          ws.clients.forEach((c) => {
            (0, import_devtools_shared2.send)(c, import_devtools_shared2.DevtoolsEvent.DEVTOOLS_DISPLAY_LOGIN_FAILURE, {
              error,
              code
            });
          });
      ...
      client.on("close", (_2, reason) => {
        ...
        if (db.clientWs) {
          if (!ws.clients.has(db.clientWs)) {
            ...
            db.activities = [];
            ws.clients.forEach((c) => {
              (0, import_devtools_shared2.send)(c, import_devtools_shared2.DevtoolsEvent.DEVTOOLS_DISCONNECTED_APP, {
                url: db.connectedApp
              });
            });
          }
